...
final class UpdateController {
    ...
    @ObservationIgnored private let driver: KBLockUserDriver
    ...
    /// `KBLOCK_UPDATE_FEED` and deliberately exercises real download/install,
    ...
    private var updatesDisabledForDevelopment: Bool {
        (ProcessInfo.processInfo.environment["KBLOCK_UPDATE_FEED"] ?? "").isEmpty
    }
    #endif

    init() {
        driver = KBLockUserDriver(model: model)
    ...
    func start() {
        ...
        do {
            ...
            if ProcessInfo.processInfo.environment["KBLOCK_UPDATE_CHECK_ON_LAUNCH"] == "1" {
                Task { [weak self] in
...
final class UpdaterDelegate: NSObject, SPUUpdaterDelegate {
    ...
    func feedURLString(for updater: SPUUpdater) -> String? {
        ...
        if let feed = ProcessInfo.processInfo.environment["KBLOCK_UPDATE_FEED"],
           !feed.isEmpty {
            return feed
        }
        #endif
        #if arch(x86_64)
        return "https://bbroot.github.io/KBLock/appcast-x86_64.xml"
        #else
        return nil
        #endif
    }
