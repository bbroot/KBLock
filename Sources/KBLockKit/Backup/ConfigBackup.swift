...
/// a backup carries between machines: the global default source and per-app
/// rules. Per-device *runtime* state (the master lock, language preference, the
/// login item) is deliberately **not** here, so importing never flips those on
...
public struct BackupPayload: Codable, Equatable, Sendable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        // Silently ignore legacy `urlRules` key
    }
}
...
///   rejected ("please update KBLock"), shown via the human `appVersion` string
...
public extension ConfigBackup {
    /// Marks a file as a KBLock backup; a file lacking it isn't ours.
    static let formatIdentifier = "com.bbroot.KBLock.backup"
    ...
    /// newer KBLock (whose payload this build may not understand) still reports
...
public enum BackupReadError: Error, Equatable, Sendable {
    ...
    case unreadable
    /// The bytes were read fine, but it isn't a KBLock backup — not JSON, or
    /// missing/wrong format identifier.
    case notABackup
    /// A KBLock backup whose contents are structurally broken.
    case damaged
    /// Written by a newer KBLock than this build can read. Carries the
