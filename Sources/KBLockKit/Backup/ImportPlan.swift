...
/// Per-conflict resolution. The vocabulary is KBLock's own ("keep local / use
...
/// One reviewable binding from an imported backup — the global default or an
/// app rule — unified so the Review screen and the resolver treat them
/// identically.
public struct ImportItem: Identifiable, Sendable, Equatable {
    ...
    /// File-side app-rule mode (`nil` for the global default).
...
public struct ImportPlan: Sendable, Equatable {
    ...
    /// from. Held so `resolvedConfiguration()` can keep `isEnabled` untouched.
    ...
    /// flags (`isEnabled`) are carried straight from the base config and never
    /// imported.
    ...
    public func summary() -> ImportSummary {
        ...
        for (key, binding) in resolvedKeys {
            ...
            // stays a true subset of the imported count.
