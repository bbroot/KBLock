...
/// surfaces that bypass SwiftUI — `NSAlert`, `NSWindow.title` — don't, so they
/// must look the string up in the chosen language's `.lproj` directly. Keys are
/// the English source strings (matching `Localizable.xcstrings`).
@MainActor
enum AppKitStrings {
    ...
    static func string(_ key: String, language: SupportedLanguage, _ arguments: CVarArg...) -> String {
