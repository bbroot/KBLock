...
/// for a mixed-language screenshot. `KBLockKit` itself is non-UI, so only
/// `Sources/KBLock` is scanned.
@Suite("Localization guards")
struct LocalizationGuardTests {
    private static let repoRoot = URL(fileURLWithPath: #filePath)
        .deletingLastPathComponent() // KBLockKitTests/
        .deletingLastPathComponent() // Tests/
        .deletingLastPathComponent()

    private static let appSources = repoRoot.appending(path: "Sources/KBLock")
